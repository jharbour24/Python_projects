...
def main():
    ...
    try:
        ...
        # Test Cloudflare bypass first (but don't block if it fails)
        cloudflare_ok = scraper.test_cloudflare_bypass()
        if not cloudflare_ok:
            logger.warning("Direct IBDB access blocked - will try Google search anyway")
            logger.warning("Individual production pages may still be accessible")
